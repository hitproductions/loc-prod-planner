...
// Who can take regular record/edit work: excludes the specials engineer (rule 7
// sends only Special projects there) and overflow-only people.
function receditPool(engineerRows) {
  return engineerRows
    .filter(e => e && e.name && yes(e.can_record) && yes(e.can_edit) &&
                 !yes(e.specials_only) && !yes(e.overflow_only))
    .map(e => e.name);
}
...
function bottleneck(bookingRows, engineerRows, series) {
  ...
  const forcedBy = {};
  for (const b of rows.filter(b => /FORCED/i.test(b.note || ''))) {
    forcedBy[b.engineer] = (forcedBy[b.engineer] || 0) + 1;
  }
