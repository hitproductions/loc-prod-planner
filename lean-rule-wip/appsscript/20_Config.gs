...
var P_COL = {
  ...
  MUSIC: 6, SPECIAL: 7, REC_PICK: 8, ED_PICK: 9, MIX_PICK: 10,
  DUB_W: 11, EDIT_W: 12, MIX_W: 13,
  RECORDIST: 14, EDITOR: 15, MIXER: 16, WARNINGS: 17, NOTES: 18, PLOTTED: 19,
  LOCKED: 20,
};
var P_INPUT_LAST  = P_COL.MIX_PICK;      // columns 1..10 are the user's
var P_OUTPUT_FIRST = P_COL.DUB_W;        // 11..19 belong to the engine
...
var P_HEADERS = [
  ...
  'Music', 'Special', 'Recordist pick', 'Editor pick', 'Mixer pick',
...
// ---- Engineers tab ----------------------------------------------------------
// specials_only replaced non_netflix_router on 2026-08-13. They were the same
...
var E_HEADERS = ['name', 'can_record', 'can_edit', 'can_mix', 'mix_level',
                 'music_specialist', 'overflow_only', 'specials_only'];
var E_HEADER_ROW = 1;
...
var SOLVE_OBJECTIVE = [
  ...
  'idle_regular_months',     // then fewest people with a whole month of nothing
...
// ---- Lean seasons -----------------------------------------------------------
// When a project's weeks each have at least this many engineers free, the book is
// quiet and EVERY phase is divided week by week — dub, edit and mix alike.
//
// Tara's ruling (2026-08-15), and it overrides the sharing policy above: in a lean
// stretch nobody can be overloaded, because most of the roster is free. Continuity
// is worth less than people having work. So SHARE_PREFER_WHOLE_EDIT and the
// never-split-the-mix rule are both suspended here, and the measure-it-both-ways
// tuner cannot undo it — in a quiet month dividing never "scores better", since
// there is no overlap for it to relieve.
//
// The threshold is on SLACK, which is how it finds lean periods without knowing
// what month it is: in a busy week two or three people are free and nothing
// changes; in a quiet one, most of the roster is.
// 0 turns the rule off entirely.
var DIVIDE_WHEN_FREE = 4;
