...
// The non_netflix_router column became specials_only, and the record/edit routing
...
const APPROVED_SUBS = {
  'engine/replan.js': [
    ...
    ['        let pool = isNetflix\n' +
     ...
     '          : Object.keys(eng).filter(n => eng[n].record && eng[n].edit && !eng[n].specials && !eng[n].overflow);\n' +
    ...
    ["        if (isMusic) mp = mp.concat(Object.keys(eng).filter(n => eng[n].music && eng[n].mix && free(n, mixw)));",
     "        if (isMusic) mp = musicPool(n => eng[n].mix).filter(n => free(n, mixw));"],
  ],
  'engine/assign.js': [
    ...
    ["  const pick = (pool, ref) => {\n    if (!pool.length) return null;\n",
     ...
     "      pool = pool.filter(n => eng[n].musicRank === best);\n" +
     "    }\n"],
    // MUSIC POOL + LEAN SEASONS. Pure insertions above forcePick: the music-specialist
    // pool (2026-08-15), the slack detector, and the week-by-week phase splitter every
    // phase uses when the roster is free. forcePick itself is untouched.
    ["  const forcePick = (pool, wks) => pool.slice().sort((a, b) =>",
  ...
  'engine/stats.js': [
    ['overflow_only: yes(e.overflow_only), non_netflix_router: yes(e.non_netflix_router) };',
     'overflow_only: yes(e.overflow_only), specials_only: yes(e.specials_only) };'],
    ["roster[n]?.non_netflix_router ? 'non-Netflix router' : ''",
     "roster[n]?.specials_only ? 'specials' : ''"],
  ],
};
