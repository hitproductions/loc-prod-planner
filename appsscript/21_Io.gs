...
// Sheets batches writes. A separate execution that reads immediately afterwards can
// see the old values, which is how the app ended up repainting the Schedule from a
// fresh call while the Projects view rendered a payload fetched a moment too early.
// Every write path ends with this.
function ioCommit_() {
...
function readEngineersUncached_() {
  ...
  trimmedRows_(sh, E_HEADER_ROW).forEach(function (r) {
    ...
    out.push({
      ...
      music_specialist: r[5], overflow_only: r[6], does_specials: r[7],
      // appended, so a roster that predates the column simply reads blank = No
      atmos: r[8],
    });
  });
...
function supersedeBookingRowNumbers(rowNumbers) {
  ...
  if (n) { col.setValues(cur); ioInvalidate('bookings'); ioCommit_(); }
  return n;
}
...
// Rewrite the Projects tab's output columns from the live bookings.
//
// The engine fills those columns at plot time, from what it just decided. A re-plan
// decides differently and writes only the Bookings tab, so Recordist / Editor /
// Mixer / Warnings / Notes — and the red highlight on a forced row — kept showing
// the plot's answer for ever after. The Schedule view reads bookings and updated;
// everything sourced from the Projects tab quietly did not, including the sheet
// itself, which the team is told they can read (HANDOFF §8).
//
// Derived from the rows rather than re-running the engine: after a re-plan the rows
// ARE the decision, and re-deriving anything else risks disagreeing with them.
function refreshProjectOutputsFromBook_(titles) {
  var wanted = {};
  (titles || []).forEach(function (t) { wanted[String(t).trim()] = true; });
  var live = activeRows(readBookings());
  var rows = readProjectRows();
  var n = 0;

  rows.forEach(function (p) {
    if (!p.project_title || (titles && !wanted[p.project_title])) return;
    var mine = live.filter(function (b) { return b.project === p.project_title; });
    if (!mine.length) return;

    var names = function (phase) {
      var seen = [];
      mine.forEach(function (b) {
        if (b.phase === phase && b.engineer && seen.indexOf(b.engineer) === -1) seen.push(b.engineer);
      });
      return seen.length ? seen.join(' + ') : '\u2014';
    };
    // from the weeks, not the note — see doubledProjects_
    var forced = mine.some(function (b) {
      for (var w = widx(b.start_date); w <= widx(b.end_date); w++) {
        var n = 0;
        live.forEach(function (o) {
          if (o.engineer === b.engineer && widx(o.start_date) <= w && w <= widx(o.end_date)) n++;
        });
        if (n > 1) return true;
      }
      return false;
    ...
    var notes = [];
    var dubbers = names('Dub');
    if (dubbers.indexOf(' + ') !== -1) notes.push('Dub divided between ' + dubbers + '.');
    if (forced) notes.push('Double-booked in at least one week.');
    notes.push('Re-planned ' + todayISO() + '.');

    writeProjectOutputs(p._row, {
      dub_weeks: p.dub_weeks, edit_weeks: p.edit_weeks, mix_weeks: p.mix_weeks,
      recordist: dubbers, editor: names('Edit'), mixer: names('Mix'),
      warnings: forced ? 'Forced overlap' : '',
      notes: notes.join(' '),
      plotted: p.plotted || todayISO(),
      forced: forced,
    });
    n++;
  });
...
// Make the FORCED note on every LIVE booking row true.
//
// The engine writes that note when it decides to double-book someone. It is a record
// of a decision, not of a fact, and it drifts the moment anything else moves: a swap
// or a divided dub resolves the collision and the note stays, while a row that BECOMES
// doubled because of someone else's placement never gets one. Every surface that read
// it was wrong in both directions.
//
// Reading them correctly elsewhere was not enough — the Bookings tab IS the record,
// and it was carrying the false claim. Superseded rows are left exactly as they were:
// those are history, and history should say what was believed at the time.
//
// One read and one write over the note column, whatever the row count.
function refreshForcedNotes_() {
  var sh = sheetOrThrow(TAB.BOOKINGS);
  var last = sh.getLastRow();
  if (last <= B_HEADER_ROW) return 0;
  var first = B_HEADER_ROW + 1;
  var height = last - B_HEADER_ROW;
  var col = sh.getRange(first, B_COL.NOTE, height, 1);
  var notes = col.getValues();
  ...
  live.forEach(function (b) {
    weeksOf(b).forEach(function (w) {
      var k = b.engineer + '|' + w;
      (per[k] = per[k] || []).push(b.row_number);
    });
  });
  var doubled = {};
  Object.keys(per).forEach(function (k) {
    if (per[k].length > 1) per[k].forEach(function (rn) { doubled[rn] = true; });
  });

  var changed = 0;
  live.forEach(function (b) {
    var i = b.row_number - first;
    if (i < 0 || i >= height) return;
    var was = String(notes[i][0] == null ? '' : notes[i][0]);
    // keep everything the note says EXCEPT its claim about overlapping
    var parts = was.split(' / ').map(function (x) { return x.trim(); })
      .filter(function (x) { return x && !/FORCED|double-?booked/i.test(x); });
    if (doubled[b.row_number]) parts.push('DOUBLE-BOOKED this week');
    var now = parts.join(' / ');
    if (now !== was) { notes[i][0] = now; changed++; }
  });
  if (changed) { col.setValues(notes); ioInvalidate(); ioCommit_(); }
  return changed;
}
...
function readProjectRowsUncached_() {
  ...
  trimmedRows_(sh, P_HEADER_ROW).forEach(function (r, i) {
    ...
    out.push({
      ...
      atmos_required: String(r[P_COL.ATMOS - 1] || '').trim(),
      recordist_override: String(r[P_COL.REC_PICK - 1] || '').trim(),
      recordist_override_2: String(r[P_COL.REC_PICK2 - 1] || '').trim(),
