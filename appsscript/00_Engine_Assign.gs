...
function runAssign(proj, bookingRows, engineerRows) {
  ...
  // Dub and edit are named separately, with their own weeks, always. They are
  // separate phases, booked separately and shown separately on the schedule, so a
  // note that merges them into "the block" hides exactly what the reader is trying
  // to work out — which phase was the problem, and in which weeks.
  const phaseSpan = (dubWks, editWks, lead) => {
    const span = wks => wks.size
      ? (wks.size === 1 ? wstart(Math.min(...wks))
                        : wstart(Math.min(...wks)) + ' to ' + wend(Math.max(...wks)))
      : null;
    const parts = [];
    const d = span(dubWks), e = span(editWks);
    if (d) parts.push(dubWks.size + 'wk dub (' + d + ')');
    if (e) parts.push(editWks.size + 'wk edit (' + e + ')');
    return parts.length ? lead + ' the ' + parts.join(' and the ') : '';
  };
  ...
  // An Atmos title can only be mixed by someone with the room. Applied to every mix
  // path — preferred, fallback, overflow and music — because a gate that only covers
  // the happy path is not a gate: the fallbacks are exactly where a busy book ends up.
  const needAtmos = yes(proj.atmos_required);
  if (manual(recOv2) && !manual(recOv))
    warnings.push('A second recordist was named without a first — set Recordist pick as well.');
  const hasAtmos = n => !needAtmos || eng[n].atmos;

  let pool;
  if (isSpecial) pool = Object.keys(eng).filter(n => eng[n].specials);
  else {
    // The specials engineer is NOT excluded from ordinary work. does_specials means
    // "specials go only to these people", not "these people do only specials" — Kyle is
    // the only one who CAN take a Valorant title, and that is no reason to keep him off
    // a Netflix dub (Tara, 2026-08-30). The old exclusion came across from the handoff's
    // non_netflix_router and was never questioned in the rename.
  ...
  // A hand-picked PAIR on the dub. The engine has divided dubs on its own since the
  // start; this is the same shape, chosen by name instead of by measurement.
  //
  // Weeks are dealt CONTIGUOUSLY — the first name takes the earlier block — not
  // alternated. Someone told they have the first two weeks can plan around it; a
  // week-on-week-off split hands two people a broken run each, which is what the
  // engine's own divider is careful to avoid. Individual weeks can still be dragged
  // afterwards if the halves are not where you wanted them.
  //
  // Only the DUB divides. The edit is deliberately kept whole (SHARE_PREFER_WHOLE_EDIT)
  // and the mix is never split, so a second name is offered for the recordist alone.
  if (manual(recOv) && manual(recOv2) && dubw.size) {
    const notes = [];
    if (recOv === recOv2) {
      warnings.push(`${recOv} is named twice for the dub — the second pick was ignored.`);
    }
    const ordered = [...dubw].sort((a, b) => a - b);
    const cut = Math.ceil(ordered.length / 2);
    const halves = (recOv === recOv2)
      ? [[recOv, new Set(ordered)]]
      : [[recOv, new Set(ordered.slice(0, cut))], [recOv2, new Set(ordered.slice(cut))]];

    const took = [];
    for (const pair of halves) {
      const name = pair[0], wks = pair[1];
      if (!wks.size) continue;
      if (!vetPick(name, wks, 'the dub', 'a recordist', e => e.record)) continue;
      commit(name, wks, d);
      // One row per contiguous run, exactly as applySpread does. A booking row is
      // emitted as min..max of its week set, so handing it a set with a gap would
      // silently claim weeks the engineer does not hold. The halves are contiguous
      // today; this makes the branch correct whatever the split turns into.
      for (const r of runsOf(wks)) assignments.push(['Dub', name, r]);
      took.push(name + ': ' + wks.size + 'wk');
    }
    notes.push(took.length > 1
      ? 'Dub divided by hand — ' + took.join(' | ') + '.'
      : 'Recordist assigned by hand.');

    // The edit follows the same rules it always did: the named editor if there is one,
    // otherwise the engine picks, keeping it whole.
    let edName = null;
    if (manual(edOv)) {
      if (!vetPick(edOv, editw, 'the edit', 'an editor', e => e.edit)) edName = 'UNRESOLVED';
      else { edName = edOv; notes.push('Editor assigned by hand.'); }
    } else if (editw.size) {
      const freeEd = pool.filter(n => free(n, editw));
      if (freeEd.length) edName = pick(freeEd, Math.min(...editw), isMusic);
      else if (pool.length) {
        edName = forcePick(pool, editw);
        notes.push(`FORCED OVERLAP on the edit — ${edName} was already committed ` +
          `${inter(eng[edName].busy, editw).length} of those weeks elsewhere.`);
      }
    }
    if (edName && edName !== 'UNRESOLVED' && editw.size) {
      commit(edName, editw, ed);
      assignments.push(['Edit', edName, editw]);
    }

    const dubbers = took.map(t => t.split(':')[0]);
    rec = !dubbers.length ? 'UNRESOLVED'
        : (editw.size && edName && dubbers.indexOf(edName) === -1)
          ? dubbers.join(' + ') + ' (dub) / ' + edName + ' (edit)'
          : dubbers.join(' + ');
    recNote = notes.join(' ');
  } else if (manual(recOv) && !manual(edOv)) {
    // one name, both phases
    if (!vetPick(recOv, recedit, 'the dub and the edit', 'a recordist or editor', e => e.record && e.edit)) rec = 'UNRESOLVED';
    else {
    ...
    // 2. one engineer for both the dub and the edit
    ...
    const spread = spreadDub(pool, dubw, editw, isMusic);
    ...
    if (dividesDub) {
      ...
      rec = applySpread(spread, editw).join(' + ');
      recNote = spreadNote(spread, editw) + phaseSpan(dubw, editw, ' — nobody was free for');
    } else if (splitOk) {
      ...
      if (dubPick === editPick) {
        ...
        recNote = 'Split by necessity —' + phaseSpan(dubw, editw, ' nobody was free for') + '.';
      ...
      recNote = `FORCED OVERLAP to hit the deadline — ${f} was already committed ${ov} of these weeks elsewhere.` +
        phaseSpan(dubw, editw, ' No engineer was free for') +
        ', and dividing the dub did not cover it either.';
  ...
  if (!mixw.size) {
    ...
    const canMix = n => eng[n].mix && !eng[n].overflow && hasAtmos(n);
    ...
    if (mixPool.length) { mixer = pick(mixPool, Math.min(...mixw)); commit(mixer, mixw, m); }
      ...
      const overflow = Object.keys(eng).filter(n => eng[n].overflow && eng[n].mix && (!needAdv || eng[n].lvl === 'Advanced') && hasAtmos(n) && free(n, mixw));
      if (overflow.length) {
        ...
        const all = regular.concat(fallback)
          .concat(Object.keys(eng).filter(n => eng[n].overflow && eng[n].mix && (!needAdv || eng[n].lvl === 'Advanced') && hasAtmos(n)));
